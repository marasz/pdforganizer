...
split_pdf_conf = config['split_pdf']
...
def split_pdf(path):
    ...
    for i in range(page_numbers):
        ...
        file_name = "{}_{}_{}.pdf".format(now.strftime("%Y-%m-%d"), now.microsecond, i + 1)
    ...
    os.rename(path, str(
        os.path.join(original_move_folder, now.strftime("%Y-%m-%d") + "_" + str(now.microsecond) + '_Scan.pdf')))
...
def tokenize_pdfs(directory):
    ...
    for subdir, dirs, files in os.walk(directory):
        for file in files:
            if str(file).endswith('.pdf'):
                if split_pdf_conf:
                    split_pdf_conf(str(os.path.join(subdir, file)))
...
def categorize_pdfs(directory):
    ...
    for file in os.listdir(directory):
        if str(file).endswith('.pdf'):
            ...
            f_dist = FreqDist(word.lower() for word in tokens)
            ranking = {}
            keywords = {}
            for folder in folder_keywords:
                ranking[folder] = sum([f_dist[keyword] for keyword in folder_keywords[folder]])
                keywords[folder] = [(keyword, f_dist[keyword]) for keyword in folder_keywords[folder]]
            ...
            if sorted_ranking[0][1] == 0:
                ...
                os.rename(str(os.path.join(directory, file)),
                          str(os.path.join(target_folder, sorted_ranking[0][0], filename)))
                if print_paths:
